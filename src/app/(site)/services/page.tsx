...
const defaultContent = {
  ...
  servicesSectionSubtitle: "From your first enquiry to graduation, we are here to help every step of the way.",
...
export default async function Services() {
  ...
  const pageData = {
    ...defaultContent,
    ...pageContent,
    serviceCards: pageContent?.serviceCards?.length ? pageContent.serviceCards : defaultServices,
    processSteps:
      pageContent?.processSteps?.length
        ? pageContent.processSteps
        : [
        ...
        <div className="container">
          <div className="text-center max-w-2xl mx-auto mb-16">
            ...
            <p className="text-muted-foreground text-lg">
              {pageData.servicesSectionSubtitle}
            </p>
          </div>

          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {pageData.serviceCards.map((service: any, index: number) => {
              const fallbackIcon = defaultServices[index]?.icon ?? MessageCircle;
              ...
              >
                ...
                <ul className="space-y-2">
                  {service.features.map((feature: any, featureIndex: number) => (
                    <li key={feature.id ?? `${feature.text}-${featureIndex}`} className="flex items-center gap-2 text-sm">
      ...
      <section className="py-20 bg-muted">
        <div className="container">
          <div className="text-center max-w-2xl mx-auto mb-16">
            ...
            <p className="text-muted-foreground text-lg">
              {pageData.processSectionSubtitle}
            </p>
          </div>

          <div className="grid md:grid-cols-4 gap-8">
            {pageData.processSteps.map((item: any, index: number) => (
